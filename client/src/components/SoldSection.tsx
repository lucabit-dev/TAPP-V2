import React from 'react';
import { useAuth } from '../auth/AuthContext';

interface SoldPosition {
  symbol: string;
  groupKey: string;
  groupLabel: string;
  avgPrice: number;
  quantity: number;
  stageIndex: number;
  stageLabel: string;
  stageDescription: string;
  stopPrice: number | null;
  limitPrice: number | null;
  orderId: string | null;
  orderStatus: string | null;
  status: string;
  statusLabel: string;
  createdAt: number | null;
  updatedAt: number | null;
  // Sold position specific fields
  sellPrice?: number;
  pnlPerShare?: number;
  totalPnL?: number;
  soldAt?: number;
}

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001/api';

const SoldSection: React.FC = () => {
  const { fetchWithAuth } = useAuth();
  const [rows, setRows] = React.useState<SoldPosition[]>([]);
  const [loading, setLoading] = React.useState<boolean>(true);
  const [error, setError] = React.useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = React.useState<number | null>(null);
  const refreshIntervalRef = React.useRef<number | null>(null);

  const fetchStatus = React.useCallback(async () => {
    try {
      const response = await fetchWithAuth(`${API_BASE_URL}/stoplimit/status`);
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `HTTP ${response.status}`);
      }

      const data = await response.json();
      if (!data.success) {
        throw new Error(data.error || 'Failed to load StopLimit status');
      }

      // Filter only sold positions
      const allRows = Array.isArray(data.data) ? data.data : [];
      const soldRows = allRows.filter((row: SoldPosition) => row.status === 'sold');
      
      // Sort by soldAt (most recent first)
      soldRows.sort((a: SoldPosition, b: SoldPosition) => {
        const aTime = a.soldAt || a.updatedAt || 0;
        const bTime = b.soldAt || b.updatedAt || 0;
        return bTime - aTime;
      });

      setRows(soldRows);
      setError(null);
      setLastUpdated(Date.now());
    } catch (err: any) {
      setError(err.message || 'Failed to load sold positions');
    } finally {
      setLoading(false);
    }
  }, [fetchWithAuth]);

  React.useEffect(() => {
    fetchStatus();
  }, [fetchStatus]);

  React.useEffect(() => {
    if (refreshIntervalRef.current) {
      clearInterval(refreshIntervalRef.current);
    }

    refreshIntervalRef.current = window.setInterval(() => {
      fetchStatus();
    }, 4000);

    return () => {
      if (refreshIntervalRef.current) {
        clearInterval(refreshIntervalRef.current);
        refreshIntervalRef.current = null;
      }
    };
  }, [fetchStatus]);

  const formatCurrency = (value: number | null | undefined) => {
    if (value === null || value === undefined || Number.isNaN(value)) return 'N/A';
    return `$${value.toFixed(2)}`;
  };

  const formatNumber = (value: number | null | undefined, decimals = 2) => {
    if (value === null || value === undefined || Number.isNaN(value)) return 'N/A';
    return value.toFixed(decimals);
  };

  const formatRelativeTime = (timestamp: number | null | undefined) => {
    if (!timestamp) return 'N/A';
    const diff = Date.now() - timestamp;
    if (diff < 0) return 'Just now';
    const seconds = Math.floor(diff / 1000);
    if (seconds < 60) return `${seconds}s ago`;
    const minutes = Math.floor(seconds / 60);
    if (minutes < 60) return `${minutes}m ago`;
    const hours = Math.floor(minutes / 60);
    if (hours < 24) return `${hours}h ago`;
    const days = Math.floor(hours / 24);
    return `${days}d ago`;
  };

  const formatTimestamp = (timestamp: number | null | undefined) => {
    if (!timestamp) return 'N/A';
    const date = new Date(timestamp);
    return date.toLocaleString();
  };

  const describeOrderStatus = (status: string | null) => {
    if (!status) return 'Unknown';
    const upper = status.toUpperCase();
    switch (upper) {
      case 'FLL':
      case 'FIL':
        return 'Filled';
      case 'DON':
        return 'Queued (DON)';
      case 'QUE':
      case 'QUEUED':
        return 'Queued';
      case 'ACK':
        return 'Acknowledged';
      case 'REC':
        return 'Received';
      case 'OUT':
        return 'Cancelled';
      case 'REJ':
        return 'Rejected';
      default:
        return upper;
    }
  };

  const getSellMethod = (row: SoldPosition) => {
    // If it has a stageIndex >= 0, it was sold via StopLimit
    if (row.stageIndex >= 0) {
      return 'StopLimit';
    }
    // Otherwise, it was likely sold manually
    return 'Manual';
  };

  const summary = React.useMemo(() => {
    const total = rows.length;
    const totalPnL = rows.reduce((sum, row) => sum + (row.totalPnL || 0), 0);
    const profitable = rows.filter(row => (row.totalPnL || 0) > 0).length;
    const loss = rows.filter(row => (row.totalPnL || 0) < 0).length;
    const stopLimitSales = rows.filter(row => row.stageIndex >= 0).length;
    const manualSales = rows.filter(row => row.stageIndex < 0).length;

    return { total, totalPnL, profitable, loss, stopLimitSales, manualSales };
  }, [rows]);

  return (
    <div className="h-full flex flex-col bg-[#14130e]">
      <div className="p-4 border-b border-[#2a2820]/60 bg-gradient-to-r from-[#14130e] to-[#0f0e0a]">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-3 lg:space-y-0">
          <div>
            <h2 className="text-sm font-bold text-[#eae9e9] tracking-wider uppercase">Sold Positions</h2>
            <p className="text-xs text-[#eae9e9]/60 mt-1">
              History of all positions sold via StopLimit or manually
            </p>
          </div>
          <div className="flex flex-col items-start lg:items-end space-y-2">
            <div className="text-[11px] text-[#eae9e9]/60">
              Last update: {lastUpdated ? `${formatRelativeTime(lastUpdated)} (${formatTimestamp(lastUpdated)})` : 'N/A'}
            </div>
            <div className="flex items-center space-x-3">
              <button
                onClick={() => fetchStatus()}
                className="px-3 py-1.5 text-xs font-semibold rounded border border-[#2a2820] text-[#eae9e9]/80 hover:text-[#eae9e9] hover:border-[#eae9e9]/30 transition-colors"
              >
                Refresh
              </button>
            </div>
          </div>
        </div>
      </div>

      <div className="px-3 py-2 border-b border-[#2a2820]/40">
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          <SummaryCard
            title="Total Sold"
            value={summary.total}
            subtitle={`${summary.stopLimitSales} StopLimit • ${summary.manualSales} Manual`}
          />
          <SummaryCard
            title="Total P&L"
            value={formatCurrency(summary.totalPnL)}
            subtitle={`${summary.profitable} profitable • ${summary.loss} loss`}
            valueColor={summary.totalPnL >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}
          />
          <SummaryCard
            title="Profitable"
            value={summary.profitable}
            subtitle={`${summary.total > 0 ? ((summary.profitable / summary.total) * 100).toFixed(1) : 0}% win rate`}
            valueColor="text-[#4ade80]"
          />
          <SummaryCard
            title="Losses"
            value={summary.loss}
            subtitle={`${summary.total > 0 ? ((summary.loss / summary.total) * 100).toFixed(1) : 0}% loss rate`}
            valueColor="text-[#f87171]"
          />
        </div>
      </div>

      {error && (
        <div className="px-4 pt-3">
          <div className="bg-[#3a1e1e] border border-[#f87171] text-[#f87171] text-xs rounded p-3">
            {error}
          </div>
        </div>
      )}

      <div className="flex-1 overflow-hidden">
        {loading ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              <div className="w-12 h-12 bg-[#2a2820] rounded-lg flex items-center justify-center mx-auto mb-4">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#4ade80]"></div>
              </div>
              <h3 className="text-sm font-medium text-[#eae9e9] mb-1">Loading sold positions...</h3>
              <p className="text-xs text-[#eae9e9]/60">Fetching the latest sales history</p>
            </div>
          </div>
        ) : rows.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center text-[#eae9e9]/70">
              <div className="w-12 h-12 bg-[#2a2820] rounded-lg flex items-center justify-center mx-auto mb-4">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <h3 className="text-base font-semibold text-[#eae9e9]">No sold positions yet</h3>
              <p className="text-xs mt-1">Sold positions will appear here once positions are closed.</p>
            </div>
          </div>
        ) : (
          <div className="h-full flex flex-col">
            <div className="bg-[#14130e] border-b border-[#2a2820]/40 px-4 py-2 sticky top-0 z-10">
              <div className="grid grid-cols-12 gap-2 text-[10px] font-semibold text-[#eae9e9]/50 uppercase tracking-wider">
                <div className="col-span-2">Symbol</div>
                <div className="col-span-1 text-center">Group</div>
                <div className="col-span-2">Sale Info</div>
                <div className="col-span-2">P&L</div>
                <div className="col-span-2">Prices</div>
                <div className="col-span-1 text-center">Quantity</div>
                <div className="col-span-2">Order / Sold</div>
              </div>
            </div>
            <div className="flex-1 overflow-auto">
              <div className="divide-y divide-[#2a2820]/50">
                {rows.map((row) => {
                  const orderStatusLabel = describeOrderStatus(row.orderStatus);
                  const sellMethod = getSellMethod(row);
                  const pnlColor = (row.totalPnL || 0) >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]';

                  return (
                    <div
                      key={`${row.symbol}-${row.soldAt || row.updatedAt}`}
                      className="px-4 py-3 bg-[#14130e] hover:bg-[#1e1d17] transition-colors"
                    >
                      <div className="grid grid-cols-12 gap-2 items-center text-xs text-[#eae9e9]/80">
                        <div className="col-span-2 flex items-center space-x-3">
                          <div className="w-2 h-2 rounded-full bg-[#4ade80]"></div>
                          <div>
                            <div className="text-sm font-semibold text-[#eae9e9]">{row.symbol}</div>
                            <div className="flex items-center space-x-2 mt-1">
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-[#1e3a2e] text-[#4ade80] border border-[#4ade80]/50">
                                SOLD
                              </span>
                              <span className="text-[10px] text-[#eae9e9]/50">
                                {sellMethod}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="col-span-1 text-center">
                          <div className="font-semibold text-[#eae9e9]">{row.groupLabel}</div>
                          <div className="text-[10px] text-[#eae9e9]/50 uppercase tracking-wider">Group {row.groupKey}</div>
                        </div>
                        <div className="col-span-2">
                          <div className="font-semibold text-[#eae9e9]">{sellMethod}</div>
                          {row.stageIndex >= 0 && (
                            <div className="text-[10px] text-[#eae9e9]/60">
                              Stage: {row.stageLabel}
                            </div>
                          )}
                          <div className="text-[10px] text-[#eae9e9]/50 mt-1">
                            Status: {orderStatusLabel}
                          </div>
                        </div>
                        <div className="col-span-2">
                          <div className={`font-mono font-semibold text-lg ${pnlColor}`}>
                            {formatCurrency(row.totalPnL)}
                          </div>
                          <div className={`text-[10px] font-medium ${row.pnlPerShare !== undefined && row.pnlPerShare >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}`}>
                            {row.pnlPerShare !== undefined ? `${row.pnlPerShare >= 0 ? '+' : ''}${formatCurrency(row.pnlPerShare)}/share` : 'N/A'}
                          </div>
                          <div className="text-[10px] text-[#eae9e9]/50 mt-1">
                            {row.totalPnL !== undefined && row.totalPnL >= 0 ? 'Profit' : 'Loss'}
                          </div>
                        </div>
                        <div className="col-span-2">
                          <div className="flex items-center space-x-2">
                            <span className="text-[10px] text-[#eae9e9]/50 uppercase tracking-wider">Buy Avg</span>
                            <span className="font-mono text-sm text-[#eae9e9]">{formatCurrency(row.avgPrice)}</span>
                          </div>
                          <div className="flex items-center space-x-2 mt-1">
                            <span className="text-[10px] text-[#eae9e9]/50 uppercase tracking-wider">Sell Price</span>
                            <span className="font-mono text-sm text-[#4ade80]">{formatCurrency(row.sellPrice)}</span>
                          </div>
                          {row.stageIndex >= 0 && row.stopPrice && row.limitPrice && (
                            <div className="flex items-center space-x-2 mt-1">
                              <span className="text-[10px] text-[#eae9e9]/50">Stop/Limit</span>
                              <span className="font-mono text-[10px] text-[#eae9e9]/70">
                                {formatCurrency(row.stopPrice)}/{formatCurrency(row.limitPrice)}
                              </span>
                            </div>
                          )}
                        </div>
                        <div className="col-span-1 text-center">
                          <div className="font-semibold text-[#eae9e9]">
                            {row.quantity ? row.quantity.toLocaleString() : 'N/A'}
                          </div>
                          <div className="text-[10px] text-[#eae9e9]/50 mt-1">Shares</div>
                        </div>
                        <div className="col-span-2">
                          <div className="font-mono text-xs text-[#eae9e9] truncate">
                            {row.orderId || 'N/A'}
                          </div>
                          <div className="text-[10px] text-[#eae9e9]/50 mt-1">
                            Sold {row.soldAt ? formatRelativeTime(row.soldAt) : 'N/A'}
                          </div>
                          <div className="text-[10px] text-[#eae9e9]/45 mt-1">
                            {row.soldAt ? formatTimestamp(row.soldAt) : 'N/A'}
                          </div>
                        </div>
                      </div>
                    </div>
                  );
                })}
              </div>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

interface SummaryCardProps {
  title: string;
  value: string | number;
  subtitle: string;
  valueColor?: string;
}

const SummaryCard: React.FC<SummaryCardProps> = ({ title, value, subtitle, valueColor = 'text-[#eae9e9]' }) => (
  <div className="bg-[#14130e] border border-[#2a2820]/40 rounded-md p-3 hover:border-[#2a2820]/60 transition-colors duration-200">
    <div className="text-[10px] font-medium text-[#eae9e9]/55 uppercase tracking-[0.15em]">
      {title}
    </div>
    <div className={`text-lg font-semibold ${valueColor} leading-none mt-2`}>{value}</div>
    <div className="text-[11px] text-[#eae9e9]/45 mt-2 leading-snug">{subtitle}</div>
  </div>
);

export default SoldSection;


