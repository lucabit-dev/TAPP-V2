...
// Stop-limit reconciliation: ensure every position has a stop-limit (create or update).
// Runs independently of Orders WS / FLL. Keeps stop-limit tracking alive without web open or FLL delivery.
let stopLimitReconciliationInProgress = false;
const RECONCILIATION_INTERVAL_MS = 60 * 1000; // 1 minute
setInterval(async () => {
  if (!process.env.PNL_API_KEY || stopLimitReconciliationInProgress) return;
  stopLimitReconciliationInProgress = true;
  try {
    const now = Date.now();
    let created = 0;
    let updated = 0;
    // Merge positions from cache + DB (DB fallback when cache empty e.g. different instance / WS delayed)
    const positionsToReconcile = new Map(positionsCache);
    if (cachePersistenceService) {
      try {
        const dbPositions = await cachePersistenceService.getAllPositionsFromDb();
        for (const [sym, data] of dbPositions) {
          if (!positionsToReconcile.has(sym)) {
            positionsToReconcile.set(sym, data);
            positionsCache.set(sym, data); // Seed cache so createStopLimitSellOrder can use Bid/Last
          }
        }
      } catch (e) {
        console.warn(`⚠️ [RECON] getAllPositionsFromDb failed:`, e.message);
      }
    }
    for (const [symbol, pos] of positionsToReconcile.entries()) {
      const normalizedSymbol = (symbol || '').toString().toUpperCase();
      const positionQty = Math.floor(parseFloat(pos.Quantity || 0)) || 0;
      if (positionQty <= 0) continue;
      if (recentlySoldSymbols.has(normalizedSymbol)) continue;
      const filledData = stopLimitFilledSymbols.get(normalizedSymbol);
      if (filledData && (now - filledData.timestamp) < 60000) continue; // 60s
      ...
      if (existing) {
        const leg = existing.order?.Legs?.[0];
        const stopLimitQty = parseInt(leg?.QuantityRemaining || leg?.QuantityOrdered || '0', 10) || 0;
        if (positionQty > stopLimitQty && positionQty > 0) {
          try {
            const res = await modifyOrderQuantity(existing.orderId, positionQty);
            if (res.success) {
              updated++;
              console.log(`✅ [RECON] Updated stop-limit for ${normalizedSymbol}: ${stopLimitQty} → ${positionQty}`);
            }
          } catch (e) {
            console.warn(`⚠️ [RECON] Update stop-limit ${normalizedSymbol} failed:`, e.message);
          }
      ...
      const buyPrice = parseFloat(pos.AveragePrice || pos.Bid || pos.Last || 0) || 0;
      if (buyPrice <= 0) continue;
      stopLimitCreationBySymbol.add(normalizedSymbol);
      try {
        const result = await createStopLimitSellOrder(normalizedSymbol, positionQty, buyPrice);
        if (result.success) {
          created++;
          console.log(`✅ [RECON] Created stop-limit for ${normalizedSymbol} (qty ${positionQty}, buy ${buyPrice})`);
        }
      } catch (e) {
        console.warn(`⚠️ [RECON] Create stop-limit ${normalizedSymbol} failed:`, e.message);
      } finally {
    ...
    if (created > 0 || updated > 0) {
      console.log(`📋 [RECON] Stop-limit reconciliation: ${created} created, ${updated} updated`);
    }
  } catch (err) {
    console.error(`❌ [RECON] Stop-limit reconciliation error:`, err.message);
  } finally {
    stopLimitReconciliationInProgress = false;
  }
...
async function handleManualBuyFilled(orderId, order, pending) {
  ...
  if (normalizedSymbol === 'TE' || normalizedSymbol === 'JBLU') {
    console.log(`🔍 [${normalizedSymbol}_DEBUG] ========== ${normalizedSymbol} StopLimit Creation Started ==========`);
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Order ID: ${orderId}`);
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Order Status: ${order.Status}`);
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Pending Data:`, JSON.stringify(pending, null, 2));
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Guard Status: ${stopLimitCreationBySymbol.has(normalizedSymbol)}`);
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Repository Status:`, getActiveStopLimitOrder(normalizedSymbol));
    console.log(`🔍 [${normalizedSymbol}_DEBUG] In Progress Set:`, Array.from(stopLimitCreationInProgress));
    console.log(`🔍 [${normalizedSymbol}_DEBUG] Recently Sold: ${recentlySoldSymbols.has(normalizedSymbol)}`);
    console.log(`🔍 [${normalizedSymbol}_DEBUG] StopLimit Filled: ${stopLimitFilledSymbols.has(normalizedSymbol)}`);
  }
      ...
      if (!stuckGuardCheck) {
        ...
        if (stuckNewQty > 0) {
          try {
            ...
            if (result.success) {
              console.log(`✅ [REBUY] Successfully updated StopLimit ${stuckGuardCheck.orderId} for ${normalizedSymbol} to ${stuckNewQty}`);
            } else {
              console.error(`❌ [REBUY] Failed to update StopLimit ${stuckGuardCheck.orderId} for ${normalizedSymbol}: ${result.error}`);
            }
          } catch (err) {
            console.error(`❌ [REBUY] Error updating StopLimit quantity for ${normalizedSymbol}:`, err);
          }
  ...
  if (hasRebuyPosition) {
    ...
    if (stopLimitFilledSymbols.has(normalizedSymbol)) {
      console.log(`✅ [DEBUG] ${normalizedSymbol} has position (rebuy detected) - clearing stopLimit filled tracking`);
      stopLimitFilledSymbols.delete(normalizedSymbol);
    }
  }
    ...
    if (pendingStopLimitCheck) {
      ...
      if (isPending) {
        ...
        if (newTotalQty > 0) {
          ...
          try {
            const result = await modifyOrderQuantity(pendingOrderId, newTotalQty);
            if (result.success) {
              console.log(`✅ [REBUY] Successfully updated StopLimit ${pendingOrderId} for ${normalizedSymbol} to ${newTotalQty}`);
            } else {
              console.error(`❌ [REBUY] Failed to update StopLimit ${pendingOrderId} for ${normalizedSymbol}: ${result.error}`);
            }
          } catch (err) {
            console.error(`❌ [REBUY] Error updating pending StopLimit quantity for ${normalizedSymbol}:`, err);
          }
        }
    ...
    if (doubleCheckPending) {
      ...
      if (isPendingOrActive) {
        ...
        if (doubleCheckNewTotalQty > 0) {
          ...
          try {
            const result = await modifyOrderQuantity(doubleCheckOrderId, doubleCheckNewTotalQty);
            if (result.success) {
              console.log(`✅ [REBUY] Successfully updated StopLimit ${doubleCheckOrderId} for ${normalizedSymbol} to ${doubleCheckNewTotalQty}`);
            } else {
              console.error(`❌ [REBUY] Failed to update StopLimit ${doubleCheckOrderId} for ${normalizedSymbol}: ${result.error}`);
            }
          } catch (err) {
            console.error(`❌ [REBUY] Error updating double-check StopLimit quantity for ${normalizedSymbol}:`, err);
          }
        }
